...
struct arp_cache {
    unsigned char state;         // ARP_CACHE_STATE_*
    ip_addr_t pa;                // protocol address
    uint8_t ha[ETHER_ADDR_LEN];  // hardware address
    struct timeval timestamp;    // last updated
};

static mutex_t mutex = MUTEX_INITIALIZER;
static struct arp_cache caches[ARP_CACHE_SIZE];  // ARP cache table
...
static void arp_input(const uint8_t *data, size_t len, struct net_device *dev) {
    ...
    ip_addr_t spa, tpa;
    int marge = 0;  // flag indicating whether or not to update
