...
struct net_timer {
    struct net_timer *next;   // pointer to the next timer
    struct timeval interval;  // firing interval
    struct timeval last;      // last fired time
    void (*handler)(void);    // pointer to function to be called on firing
};
...
static struct net_timer *timers;  // timer list
