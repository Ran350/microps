...
static void tcp_pcb_release(struct tcp_pcb *pcb) {
    ...
    memset(pcb, 0,
           sizeof(*pcb)); /* pcb->state is set to TCP_PCB_STATE_FREE (0) */
}
    ...
    for (pcb = pcbs; pcb < tailof(pcbs); pcb++) {
        if ((pcb->local.addr == IP_ADDR_ANY ||
            ...
            if (pcb->state == TCP_PCB_STATE_LISTEN) {
                if (pcb->foreign.addr == IP_ADDR_ANY &&
                    pcb->foreign.port == 0) {
    ...
    switch (pcb->state) {
        case TCP_PCB_STATE_LISTEN:
            ...
            if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
                ...
                /* (combined with SYN) will be processed in the SYN-RECEIVED
                 * state, */
        ...
        case TCP_PCB_STATE_SYN_SENT:
            ...
            if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
                ...
                if (pcb->snd.una > pcb->iss) {
                    ...
                    pcb->state = TCP_PCB_STATE_SYN_RECEIVED;
                    tcp_output(pcb, TCP_FLG_SYN | TCP_FLG_ACK, NULL, 0);
                    /* ignore: If there are other controls or text in the
    ...
    switch (pcb->state) {
        case TCP_PCB_STATE_ESTABLISHED:
            ...
            while (sent < (ssize_t)len) {
                ...
                if (tcp_output(pcb, TCP_FLG_ACK | TCP_FLG_PSH, data + sent,
                               slen) == -1) {
                    errorf("tcp_output() failure");
                    pcb->state = TCP_PCB_STATE_CLOSED;
                    tcp_pcb_release(pcb);
